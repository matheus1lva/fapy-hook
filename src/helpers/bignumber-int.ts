...
export class BigNumberInt {
  ...
  constructor(value?: bigint | number | string) {
    if (value === undefined) this.value = 0n;
    else if (typeof value === 'bigint') this.value = value;
    else if (typeof value === 'number') this.value = BigInt(value);
    else if (typeof value === 'string') this.value = value === '' ? 0n : BigInt(value);
    else this.value = 0n;
  }
  ...
  div(x: BigNumberInt, y?: BigNumberInt): this {
    if (y) this.value = y.value === 0n ? 0n : x.value / y.value;
    else this.value = x.value === 0n ? 0n : this.value / x.value;
    return this;
  }
