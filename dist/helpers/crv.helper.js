...
export const getCurveBoost = async (chainID, voter, gauge) => {
    ...
    const [{ result: workingBalance }, { result: balanceOf }] = await client.multicall({
        contracts: [
            ...
            { address: gauge, abi: curveGaugeAbi, functionName: 'balanceOf', args: [voter] },
        ],
    });
    ...
    const boost = new Float().div(toNormalizedAmount(new BigNumberInt().set(workingBalance ?? 0n), 18), new Float().mul(new Float(0.4), toNormalizedAmount(new BigNumberInt().set(balanceOf ?? 0n), 18)));
    return boost;
};
export const determineConvexKeepCRV = async (chainID, strategy) => {
    const client = createPublicClient({ transport: http(process.env[`RPC_FULL_NODE_${chainID}`]) });
    try {
