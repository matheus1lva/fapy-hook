...
export async function getVaultWithStrategies(chainId, vaultAddress) {
    const kong = new KongClient();
    const vault = await kong.getVault(chainId, vaultAddress);
    const strategies = (await Promise.all((vault?.strategies || []).map((s) => kong.getStrategy(chainId, s))))
        .filter((s) => s !== null)
        .map((strategy) => {
        return {
            ...strategy,
            ...
            address: (strategy.address ?? vaultAddress),
        };
    });
    if (!strategies && !vault)
        return null;
